...
class MysqliDb
{
    ...
    public function rawQuery($query, $bindParams = null)
    {
        ...
        LogWriter::Log($this, LL_INFO, "MySQL Query ".$this->_query);
        $stmt->execute();
        ...
        LogWriter::Log($this, LL_INFO, "Query done. Affected:  ".$this->count);
        if($this->_stmtError)
            LogWriter::Log($this, LL_ERROR. $this->_stmtError);
        $this->reset();
    ...
    public function query($query, $numRows = null)
    {
        ...
        $this->_stmtErrno = $stmt->errno;
        $res = $this->_dynamicBindResults($stmt);
        LogWriter::Log($this, LL_INFO, "Query done");
        if($this->_stmtError)
            LogWriter::Log($this, LL_ERROR. $this->_stmtError);
        $this->reset();
    ...
    public function update($tableName, $tableData, $numRows = null)
    {
        ...
        LogWriter::Log($this, "MySQL Query ".$this->_query);
        $status = $stmt->execute();
        $this->reset();
        $this->_stmtError = $stmt->error;
        $this->_stmtErrno = $stmt->errno;
    ...
    public function delete($tableName, $numRows = null)
    {
        ...
        $this->_stmtErrno = $stmt->errno;
        $this->reset();
        LogWriter::Log($this, LL_INFO, "Query done. Affected: ".$stmt->affected_rows);
        if($this->_stmtError)
            LogWriter::Log($this, LL_ERROR. $this->_stmtError);
        return ($stmt->affected_rows > 0);
    }

    public function packWhere()
    {
    ...
    protected function _buildCondition($operator, &$conditions)
    {
        ...
        foreach($conditions as $cond) {
            if(isset($cond[0]) && is_array($cond[0])) {
                $this->_query .= ' (';
                $this->_buildCondition("", $cond);
                $this->_query .= ')';
                continue;
            }
            if(isset($cond[1]) && is_array($cond[1])) {
                $this->_query .= " ". $cond[0] . ' (';
                foreach($cond[1] as $z => $cc) {
                    $cc = [$cc];
                    $this->_buildCondition("", $cc);
                }
                $this->_query .= ')';
                continue;
            }
            ...
            switch(strtolower($operator)) {
                case 'fulltext':
                    $this->_query .= ' AGAINST (? IN BOOLEAN MODE)';
                    $this->_bindParam($val);
                    break;
                case 'not in':
                case 'in':
    ...
    public function __destruct()
    {
        ...
        if($this->_mysqli && $this->_own_mysqli) {
            \LogWriter::Log(__METHOD__, LL_INFO, "Closing connection...");
            $this->_mysqli->close();
            $this->_mysqli = null;
        }
    }
